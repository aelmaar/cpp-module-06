...
/*   Updated: 2023/10/21 14:16:24 by ael-maar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ScalarConverter::convert(const std::string str_literal) {
    ...
    || type.end_type.length() > 1 || (!type.end_type.empty()
    && type.end_type != "f" && type.end_type != str_literal)
    ...
    precision_len = calc_precision_len(str_literal);
    std::cout << std::fixed << std::setprecision(precision_len);
...
void ScalarConverter::convertToChar(t_type_convert &type, const std::string &str_literal) {
    (void)str_literal;
    if (type.value > CHAR_MAX || type.value < CHAR_MIN || !type.is_numerical)
